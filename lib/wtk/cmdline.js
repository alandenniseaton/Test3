...
(function(p){
	...
	p.getEcho = function() {
		return ( de('table', { "class":"commandline" })
			.child(de('tbody')
				.child(de('tr')
					.child(de('td', { "class":"prompt"})
						.child(de('span', { "class":"prompt"}, [this.getPrompt()]))
					)
					.child(de('td', { "class":"input"})
						.child(de('span', { "class":"input" }, [this.getValue() ]))
					)
				)
			)
		);
