...
// special functions
//
btk.identity = function(x) { return x; };
...
(function(btk){
	...
	btk.isBoolean  = function(x) { return ostring.call(x) === BOOLEAN; };
	btk.isNumber   = function(x) { return ostring.call(x) === NUMBER; };
	btk.isString   = function(x) { return ostring.call(x) === STRING; };
	btk.isArray    = function(x) { return ostring.call(x) === ARRAY; };
	btk.isFunction = function(x) { return ostring.call(x) === FUNCTION; };

	btk.isObject   = function(x) { return Object(x) === x; };
	btk.isGlobal   = function(x) { return ostring.call(x) === "[object global]"; };
	
	btk.isDefined  = function(x) { return x !== undefined && x !== null; }
...
btk.Path = (function(btk){
	...
	function Path(elements, separator) {
		...
		if (!isString(separator)) { throw this.constructorError(); }
...
(function(btk) {
	...
	function namespace(namespace, root) {
		...
		return path(namespace, '.').ensure(root);
	};
	btk.namespace = namespace;
}(btk));
...
if (btk.config.debug) {
	btk.dmsg = function(msg) { btk.message(msg) };
} else {
...
btk.taskqueue = (function(btk) {
	...
	//--------------------------------------------------------
	function Callback(context, callback, args) {
	...
	function TaskQueue(period) {
		...
		this.period = (typeof period === "number")? period : 1;
	...
	(function(p){
		...
		p.triggerFlush = function() {
			...
			var that = this;
			setTimeout(function(){ that.flush(); }, this.period);
		};
		
		p.xxflush = function() {
			//logging causes a task to be enqueued
			//infinite loop will result if log from here too
			//dmsg("[[[" + this.className + "(" + this.id + ").flush " + this.queue.length);
			console.info([
				"[[[",
				this.className, "(", this.id, ").flush ",
				this.queue.length,
				"]]]"
			].join(""));
		...
		p.flush = function() {
			...
			console.info([
				"[[[",
				this.className, "(", this.id, ").flushed ",
				i,
				"]]]"
			].join(""));
			*/
...
btk.defer = (function(btk){
	...
	function defer(context, callback, args) {
		taskQueue.add(new Callback(context, callback, args));
	};

	var setTimeout = btk.setTimeout;
	
	function xdefer(context, callback, args) {
		setTimeout(
			function() {
...
btk.Publisher = (function(btk){
	...
	(function(p){
		...
		p.fromFunctions = function(functions) {
			...
			if (length > 0) { this.on(OK     , functions[0]); }
			if (length > 1) { this.on(ERROR  , functions[1]); }
			if (length > 2) { this.on(INFO   , functions[2]); }
			if (length > 3) { this.on(DEFAULT, functions[3]); }
		};
	...
	(function(p){
		...
		p.send = function(client, message) {
			defer(client, client.receive, [message]);
		};
...
btk.topic = (function(btk){
	...
	function block(spath, data) {
		fpublish(spath, { data:data, type:OK, block:true, trace:true });
	}
	exports.block = block;
	
	
	function trace(spath, data) {
		fpublish(spath, { data:data, type:OK, trace:true });
	}
	exports.trace = trace;
...
btk.module = (function(btk, kob, kto){
	...
	var state = (function() {
		...
		state.forum = function forum() { return FORUM; };
	...
	function require(spec) {
		...
		if (!(spec instanceof Spec)) { return; }
...
(function(btk, exp){
	...
	function injectScript(name, href, onload, onerror) {
		...
		obj.onload = function() { loaded("load"); };
		obj.onreadystatechange = function() { loaded("readystatechange"); };
		obj.onerror = function() {	onerror(name, href); };
		
		obj.type = "text/javascript";
		obj.setAttribute('btkid', name);
...
(function(btk){
	...
	if (document.addEventListener) {
		window.addEventListener (
			'load',
			function() { btk.trigger.state('window.loaded'); },
			false
		);
		...
		document.addEventListener (
			'DOMContentLoaded',
			function() { btk.trigger.state('dom.content.loaded'); },
			false
		);
...
btk.define({ name:'btk@btk', init:btk });

// don't really need these but they can make things simpler

btk.define({ name:'btk.object@btk', init:btk.object });

btk.define({ name:'btk.taskqueue@btk', init:btk.taskqueue });

btk.define({ name:'btk.Publisher@btk', init:function(){ return btk.Publisher; } });

btk.define({ name:'btk.topic@btk', init:btk.topic });

btk.define({ name:'btk.module@btk', init:btk.module });

btk.define({ name:'btk.inject@btk', init:btk.inject });
