...
btk.defer = (function(btk){
	...
	function tq_defer(context, callback, args) {
	...
	function to_defer(context, callback, args) {
		setTimeout(
			function() {
	...
	var pm_defer = (function() {
		...
		var origin = btk.document.location.origin;
		...
		var defer = function(context, callback, args) {
			q.push(new Callback(context, callback, args));
			window.postMessage(key, origin);
		};
...
btk.module = (function(btk, kob, kto){
	...
	function define(atts) {
		...
		var s = kto.subscribe(
			...
			function() {
				s.cancel();
			//	btk.defer(null, function(){ s.cancel(); }, []);
				module.libs = {};
				module.link();
				module.waitResolved();
			}
