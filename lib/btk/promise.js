...
function PWhile(piterator, ptransform, context) {
	if (!(piterator instanceof PIterator)) {
		throw new TypeError('PWhile: expected iterator:PIterator, transform:Function [, context:Any]');
	}
	
	if (!btk.isFunction(ptransform)) {
		throw new TypeError('PWhile: expected transform:Function');
	}
	
	PWhile.SUPERCLASS.call(this);
	
	this.it = piterator;
	this.tform = ptransform;
	this.ctxt = context;
...
(function(p){
	...
	function step() {
		...
		if ( !this.it.done() ) {
			...
			wrap(this.tform.call(this.ctxt, this.it.value(), this)).then({
			
				'ok': function(result) {
					this.set(this.progress, this.wrap(result));
	...
	p.stop = function() {
		this.stopped = true;
		this.set('stop', this.wrap(), true, true);
